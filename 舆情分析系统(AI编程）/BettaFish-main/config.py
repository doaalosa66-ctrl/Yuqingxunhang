...
class Settings(BaseSettings):
    ...
    MAX_REFLECTIONS: int = Field(1, description="最大反思次数")
    MAX_PARAGRAPHS: int = Field(3, description="最大段落数")
    MAX_TOKENS_PER_TASK: int = Field(50000, description="单次任务最大Token消耗，超过自动熔断")
    SEARCH_TIMEOUT: int = Field(240, description="单次搜索请求超时")
    MAX_CONTENT_LENGTH: int = Field(500000, description="搜索最大内容长度")

    # ================== 缓存配置 ====================
    CACHE_ENABLED: bool = Field(True, description="是否启用报告缓存，相同查询在TTL内直接返回缓存结果")
    CACHE_TTL_HOURS: float = Field(6.0, description="报告缓存有效期（小时），超过后重新生成")
...
def reload_settings() -> Settings:
    ...
    """
    global settings
    # 重新计算 .env 路径，确保读取最新文件内容
    cwd_env = Path.cwd() / ".env"
    env_file = str(cwd_env if cwd_env.exists() else (PROJECT_ROOT / ".env"))
