...
class DeepSearchAgent:
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        if not search_results:
            logger.warning(f"  ⚡ 物理熔断：段落[{paragraph_index}]搜索结果为空，跳过LLM总结，直接写入占位摘要")
            paragraph.research.latest_summary = "暂未获取到相关数据，跳过分析。"
            return
        # ===== 熔断结束 =====
    ...
    def _reflection_loop(self, paragraph_index: int):
        ...
        # ===== 空数据物理熔断 =====
        if paragraph.research.latest_summary == "暂未获取到相关数据，跳过分析。":
            logger.warning(f"  ⚡ 物理熔断：段落[{paragraph_index}]无初始数据，跳过全部反思循环")
            return
        # ===== 熔断结束 =====
