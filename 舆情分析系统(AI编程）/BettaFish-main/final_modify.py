...
# 1. 修复initSearchHints函数的缩进 - 第5046行左右
# 先找到函数位置
for i, line in enumerate(lines):
    if 'function initSearchHints' in line:
        print("找到initSearchHints在第", i+1, "行")
        # 替换这一行和后面的几行
        lines[i] = '        function initSearchHints() {\n'
        lines[i+1] = '            renderRandomSearchHints();\n'
        lines[i+2] = '        }\n'
        break

# 2. 修改历史报告抽屉标题 - 第1939行左右
for i, line in enumerate(lines):
    if '🕐 历史巡航记录' in line:
        print("找到历史巡航记录在第", i+1, "行")
        lines[i] = line.replace('🕐 历史巡航记录', '🕐 历史报告记录')
        break
...
if search_start != -1:
    ...
    if search_end != -1:
        ...
        # 替换
        lines = lines[:search_start] + new_search_lines + lines[search_end+1:]
...
if guide_start != -1:
    ...
    for i in range(guide_start, len(lines)):
        if '&lt;/button&gt;' in lines[i]:
            guide_end = i
            print("找到使用指引按钮结束在第", i+1, "行")
            break
    
    if guide_end != -1:
        # 在guide_start之前插入历史报告按钮
        history_button_lines = [
            '    &lt;!-- 右下角悬浮球：历史报告 --&gt;\n',
        ...
        # 插入
        lines = lines[:guide_start] + history_button_lines + lines[guide_start:]
