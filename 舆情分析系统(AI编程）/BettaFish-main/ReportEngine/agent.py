...
class ReportAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        self.mini_llm_client = self._initialize_mini_llm()
    ...
    def _initialize_mini_llm(self) -> LLMClient:
        """
        初始化 Map-Reduce 压缩阶段专用的轻量小模型客户端。

        优先读取 MAP_REDUCE_MINI_* 配置；若未配置或初始化失败，
        自动降级返回主模型客户端，保证流程不中断。
        """
        api_key = self.config.MAP_REDUCE_MINI_API_KEY
        model_name = self.config.MAP_REDUCE_MINI_MODEL_NAME
        base_url = self.config.MAP_REDUCE_MINI_BASE_URL

        if not api_key or not model_name:
            logger.info("[MiniLLM] 未配置小模型，Map 阶段将降级使用主模型")
            return self.llm_client

        try:
            client = LLMClient(api_key=api_key, model_name=model_name, base_url=base_url)
            logger.info(f"[MiniLLM] 小模型已初始化: {model_name}")
            return client
        except Exception as exc:
            logger.warning(f"[MiniLLM] 小模型初始化失败，降级使用主模型: {exc}")
            return self.llm_client
    ...
    def generate_report(self, query: str, reports: List[Any], forum_logs: str = "",
        ...
        normalized_reports = self._map_reduce_reports(normalized_reports, query)
        ...
        try:
            ...
            def _generate_one_chapter(section: TemplateSection, node: ChapterGenerationNode):
                """在线程池中生成单个章节，保留完整重试逻辑。返回 (order, chapter_payload)。"""
                ...
                def chunk_callback(delta: str, meta: Dict[str, Any]):
                    emit('chapter_chunk', {
                        'chapterId': meta.get('chapterId') or section.chapter_id,
                        'title': meta.get('title') or section.title,
                        'delta': delta
                    })
                ...
                # 线程安全地更新进度计数并推送事件
                nonlocal completed_chapters_count
                with completed_lock:
                    completed_chapters_count += 1
                    current_done = completed_chapters_count
                chapter_progress = 20 + round(80 * current_done / total_chapters)
                emit('progress', {
                    'progress': chapter_progress,
                    'message': f'章节 {current_done}/{total_chapters} 已完成'
                })
                completion_status = {
                    'chapterId': section.chapter_id,
                    'title': section.title,
                ...
                return section.order, chapter_payload

            # 并发生成所有章节，最大并发数由配置或环境变量控制
            max_workers = int(os.getenv('CHAPTER_MAX_WORKERS', min(total_chapters, 4)))
            logger.info(f"并发生成 {total_chapters} 个章节，max_workers={max_workers}")
            order_to_payload: Dict[int, Dict[str, Any]] = {}
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                # 每个 future 绑定一个独立的 ChapterGenerationNode 实例，
                # 避免共享 _rescue_attempted_labels 等可变状态导致竞争
                def _submit(sec: TemplateSection):
                    ...
                    return pool.submit(_generate_one_chapter, sec, node)

                futures = {_submit(sec): sec for sec in sections}
                for future in as_completed(futures):
                    order, payload = future.result()  # 任一章节失败会在此处重新抛出
                    order_to_payload[order] = payload

            # 按原始 order 顺序拼接章节，保证 IR 结构正确
            chapters = [order_to_payload[sec.order] for sec in sections]
    ...
    # ------------------------------------------------------------------ #
    #  Map-Reduce 压缩：将三路报告分块摘要后再整合，避免上下文撑爆         #
    # ------------------------------------------------------------------ #
    _MAP_REDUCE_CHUNK_SIZE = 5000   # 每块最大字符数（可通过环境变量覆盖）
    _MAP_REDUCE_THRESHOLD  = 2500   # 单路报告超过此长度才触发压缩（降低阈值，更激进压缩）

    _MAP_SYSTEM_PROMPT = (
        "你是一位专业的信息提炼助手。请将以下原始报告片段提炼为一份约800字的高密度事实要点（Fact-sheet）。"
        "要求：保留所有关键数据、核心事件、重要论点和明确结论；去除重复、冗余与无实质内容的表述；"
        "以简洁的要点形式输出，每条要点直接陈述事实，不加任何前缀或解释。"
    )
    _REDUCE_SYSTEM_PROMPT = (
        "你是一位专业的报告整合助手。以下是同一主题的多个分块摘要，"
        "请将它们整合为一份连贯、完整的综合摘要，保留所有重要信息，"
        "消除重复，确保逻辑流畅。直接输出整合后的摘要，不要加任何前缀。"
    )

    def _map_reduce_reports(
        self, normalized_reports: Dict[str, str], query: str
    ) -> Dict[str, str]:
        """
        对三路引擎报告执行 Map-Reduce 压缩。

        - 若某路报告长度 <= _MAP_REDUCE_THRESHOLD，直接保留原文。
        - 超过阈值时：将文本按 _MAP_REDUCE_CHUNK_SIZE 分块，
          Map 阶段使用轻量小模型（mini_llm_client）对每块提炼约800字 Fact-sheet，
          Reduce 阶段使用主模型将所有 Fact-sheet 整合为最终摘要。
        - 若小模型未配置或调用失败，自动降级使用主模型，不阻断流程。
        """
        chunk_size = int(
            os.getenv("MAP_REDUCE_CHUNK_SIZE", self._MAP_REDUCE_CHUNK_SIZE)
        )
        threshold = int(
            os.getenv("MAP_REDUCE_THRESHOLD", self._MAP_REDUCE_THRESHOLD)
        )
        ...
        engine_labels = {
            "query_engine":   "新闻采集引擎",
            "media_engine":   "媒体采集引擎",
            "insight_engine": "本地数据库引擎",
        }

        for key, text in normalized_reports.items():
            if not text or len(text) <= threshold:
                result[key] = text
                logger.info(
                    f"[MapReduce] {key} 长度={len(text)}，未超阈值，跳过压缩"
                )
                continue

            label = engine_labels.get(key, key)
            logger.info(
                f"[MapReduce] {key} 长度={len(text)}，超过阈值{threshold}，开始分块压缩"
            )

            # ---- Map：分块提炼 Fact-sheet（使用小模型，降级兜底主模型） ----
            chunks = [text[i: i + chunk_size] for i in range(0, len(text), chunk_size)]
            logger.info(f"[MapReduce] {key} 分为 {len(chunks)} 块，逐块提炼 Fact-sheet...")
            chunk_summaries: List[str] = []
            for idx, chunk in enumerate(chunks, 1):
                user_prompt = (
                    f"查询主题：{query}\n"
                    f"来源：{label}（第{idx}/{len(chunks)}块）\n\n"
                    f"{chunk}"
                )
                # 优先用小模型，失败则降级主模型
                for attempt_client, client_label in [
                    (self.mini_llm_client, "小模型"),
                    (self.llm_client, "主模型(降级)"),
                ]:
                    try:
                        summary = attempt_client.invoke(
                            self._MAP_SYSTEM_PROMPT, user_prompt
                        )
                        chunk_summaries.append(summary.strip())
                        logger.info(
                            f"[MapReduce] {key} 第{idx}块 Fact-sheet 完成"
                            f"（{client_label}），长度={len(summary)}"
                        )
                        break
                    except Exception as e:
                        if attempt_client is self.llm_client:
                            # 主模型也失败，截取原文兜底
                            logger.warning(
                                f"[MapReduce] {key} 第{idx}块主模型也失败，截取原文: {e}"
                            )
                            chunk_summaries.append(chunk[:1000])
                        else:
                            logger.warning(
                                f"[MapReduce] {key} 第{idx}块小模型失败，尝试降级: {e}"
                            )
                            # 若小模型与主模型是同一实例（未配置小模型时），直接截取原文
                            if self.mini_llm_client is self.llm_client:
                                chunk_summaries.append(chunk[:1000])
                                break

            # ---- Reduce：整合所有 Fact-sheet（使用主模型） ----
            if len(chunk_summaries) == 1:
                result[key] = chunk_summaries[0]
                logger.info(f"[MapReduce] {key} 只有1块，直接使用 Fact-sheet，长度={len(result[key])}")
            else:
                combined = "\n\n---\n\n".join(
                ...
                try:
                    final_summary = self.llm_client.invoke(
                        self._REDUCE_SYSTEM_PROMPT, reduce_prompt
                    )
                    result[key] = final_summary.strip()
                    logger.info(
                        f"[MapReduce] {key} Reduce完成，压缩前={len(text)}，压缩后={len(result[key])}"
                    )
                except Exception as e:
                    logger.warning(f"[MapReduce] {key} Reduce失败，使用分块摘要拼接: {e}")
                    result[key] = combined
