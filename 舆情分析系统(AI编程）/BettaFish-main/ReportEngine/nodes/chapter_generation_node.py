...
class ChapterGenerationNode(BaseNode):
    ...
    def run(
        ...
        **kwargs,
        ...
        _turbo = os.getenv('CHAPTER_TURBO_TEST', '0').strip() == '1'
        try:
            chapter_json = self._parse_chapter(raw_text)
        except ChapterJsonParseError as parse_error:
            if _turbo:
                # 极速测试模式：跳过所有修复，直接插入占位章节，不浪费任何 LLM 调用
                logger.warning(f"[TURBO] {section.title} JSON解析失败，直接占位跳过: {parse_error}")
                chapter_json = {
                    "chapterId": section.chapter_id,
                    "title": section.title,
                    "anchor": section.slug,
                    ...
                    "summary": "极速测试占位章节",
                    "blocks": [
                        {"type": "heading", "level": 2, "text": section.title, "anchor": section.slug},
                        {"type": "paragraph", "inlines": [{"text": "[TURBO] 极速测试占位，链路验证用。"}]},
                    ],
                }
                ...
                if recovered:
                    ...
                    if not placeholder:
                        logger.error(f"⚡ 终极兜底：{section.title} 所有修复均失败，插入空白占位章节")
                        chapter_json = {
                            "chapterId": section.chapter_id,
                            "title": section.title,
                            "anchor": section.slug,
                            "order": section.order,
                            "summary": "由于输出长度限制，本章节内容生成被截断，请尝试缩小分析范围重新生成。",
                            "blocks": [
                                {"type": "heading", "level": 2, "text": section.title, "anchor": section.slug},
                                {"type": "callout", "tone": "warning", "title": "章节生成异常",
                                 "blocks": [{"type": "paragraph", "inlines": [
                                     {"text": "该章节因大模型输出截断导致JSON解析失败，已跳过。请尝试缩小时间范围或简化查询后重新分析。"}
                                 ]}]},
                            ],
                        }
                        placeholder_created = True
        ...
        if not valid and errors and not _turbo:
            # 极速测试模式跳过 LLM 结构修复
        ...
        if valid and not placeholder_created and not _turbo:
            # 极速测试模式跳过内容密度检查（50字必然不达标）
            try:
        ...
        **kwargs,
        ...
        for attempt in range(1, max_stream_retries + 1):
            ...
            try:
                ...
                logger.warning(f"流式章节生成失败 (第{attempt}/{max_stream_retries}次): {stream_err}")
                if attempt < max_stream_retries:
                    logger.info("等待5秒后重试...")
                    import time as _time
                    _time.sleep(5)
                else:
                    # 流式全部失败，fallback 到非流式
                    logger.warning("流式重试耗尽，切换为非流式调用...")
                    try:
                        ...
                        logger.error(f"非流式 fallback 也失败: {invoke_err}")
                        raise invoke_err
        return ""
