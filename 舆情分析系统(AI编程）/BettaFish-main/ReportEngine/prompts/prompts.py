...
# 分章节JSON生成系统提示词
# 注意：若环境变量 CHAPTER_TURBO_TEST=1，build_chapter_user_prompt 会在用户消息头部追加极速指令，
# 此处系统提示词本身不做条件分支，保持单一定义。
...
要求：
1. 总字数约8000字，可上下浮动10%，并给出 globalGuidelines 说明整体详略策略；
...
def build_chapter_user_prompt(payload: dict) -> str:
    ...
    若环境变量 CHAPTER_TURBO_TEST=1，在消息头部追加极速指令，
    强制 LLM 每章只输出 50 字，用于链路验证。
    """
    base = json.dumps(payload, ensure_ascii=False, indent=2)
    if os.getenv('CHAPTER_TURBO_TEST', '0').strip() == '1':
        turbo_prefix = (
            "【极速测试模式】本次为链路验证，不需要真实内容。\n"
            "每个章节正文总字数严格不超过 50 个汉字，只写一句话摘要即可，禁止长篇大论。\n"
            "直接输出合法 JSON，不要任何解释。\n\n"
        )
        return turbo_prefix + base
    return base
