...
class Settings(BaseSettings):
    ...
    # Mini Model：用于 Map-Reduce 压缩阶段的轻量模型（如 DeepSeek-V3）
    # 未配置时自动降级使用主模型，不影响正常流程
    MAP_REDUCE_MINI_API_KEY: Optional[str] = Field(
        None, description="Map-Reduce 压缩阶段小模型 API 密钥（未配置则降级用主模型）"
    )
    MAP_REDUCE_MINI_BASE_URL: Optional[str] = Field(
        None, description="Map-Reduce 压缩阶段小模型 Base URL"
    )
    MAP_REDUCE_MINI_MODEL_NAME: Optional[str] = Field(
        None, description="Map-Reduce 压缩阶段小模型名称，如 deepseek-v3"
    )
